...
async function init(output) {
    ...
    const template = await vite.transformIndexHtml(
      ...
      <html lang="en">
        <head>
          ...
          <title>X-File of Web3 Universe</title>
  ...
  app.get("/watch/:mtime", async (req, res) => {
    ...
    const fileMtime = fs.statSync("contract/EnsTitle.sol").mtime.getTime();
    ...
    var watcher = fs.watch("contract/EnsTitle.sol", "utf-8", (event, filename) => {
      ...
      compile().then(
        (output) =>
          res.json({
            changed: true,
            output,
            mtime: fs.statSync("contract/EnsTitle.sol").mtime.getTime(),
          }),
        (error) => console.error(error)
      );
    });
...
async function compile() {
  ...
  const sources = {};
  for (const src of fs.readdirSync(path.join(__dirname, "../contract/"))) {
    if (src.endsWith(".sol")) {
      sources[src] = {
        content: fs.readFileSync("./contract/"+src, "utf8"),
      };
    }
  }
